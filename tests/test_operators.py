...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(-0.1) < sigmoid(0) < sigmoid(0.1)
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    # TODO: Implement for Task 0.2.
    flag_1 = lt(a, b) and lt(b, c)
    flag_2 = lt(a, c)
    assert not (flag_1 and not flag_2)
...
def test_other(a: float, b: float, c: float) -> None:
    ...
    assert_close(add(a, 0.0), a)
    assert_close(add(a, neg(a)), 0.0)
    assert_close(mul(a, 1.0), a)
    assert_close(mul(a, 0.0), 0.0)
